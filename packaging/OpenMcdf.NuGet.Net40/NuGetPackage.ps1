...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
	$packageTask = Create-Process .\NuGet.exe ("pack Net40.nuspec -Symbol -Verbosity Detailed")
...
Else {
	$packageTask = Create-Process .\NuGet.exe ("pack Net40.nuspec -Verbosity Detailed")
	$packageTask.Start() | Out-Null
	$packageTask.WaitForExit()
